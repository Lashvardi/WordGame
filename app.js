    ...
    $.ajax({
      ...
      success: function (result) {
          ...
          (response) => {
            if (response.status == 200) {
              ...
              $.ajax({
                ...
                url: "https://api.api-ninjas.com/v1/randomword?type",
                headers: {
                  "X-Api-Key": "VI6ivRV3/4F56OSmro3Dow==iDcEphCADVrguHal",
                },
                contentType: "application/json",
                success: function (result) {},
        ...
        function colorEm() {
          ...
          var end =
            Math.floor(Math.random() * ($div.length - start)) + start + 2;
          if (end === $div.length) {
            end = undefined;
          }
          $div.css("background", "");
        ...
        setTimeout(function () {
          var MissingWord = $("#Push_here .Missing")
            .map(function () {
          ...
          $("#GetInput").keyup(function (event) {
            if (event.keyCode === 13) {
              ...
              if ($missing.length === 0) {
                alert("You Won! Press Space for new word");
              }
            }
          });
